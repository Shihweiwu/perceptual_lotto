function myscreen = testExperiment_dotCatch_limitLife_3(screenName)
%
% Modified from testExperiment. dot-catching task.
% > Limit dots lifespan (e.g. 6 frames for 60Hz refresh rate = 100ms) so
% that after every 6 frames we will redetermine all dots' motion path by resampling from
% the same von Mises. Every time motion path is set and reset, add random
% noise to the angle sampled from von Mises.
%
...
task{2}{1}.parameter.rndAngle = pi/2;
% initialize our task
...
stimulus.dots.rndAngle = task.thistrial.rndAngle; %boundary for adding random angle to the angle selected from von Mises.
...
% stimulus.dots.x = stimulus.dots.xcenter + unifrnd(-stimulus.dots.width/2,stimulus.dots.width/2,1,stimulus.dots.n);
% stimulus.dots.y = stimulus.dots.ycenter + unifrnd(-stimulus.dots.height/2,stimulus.dots.height/2,1,stimulus.dots.n);
...
theta = theta + unifrnd(stimulus.dots.mu-stimulus.dots.rndAngle,stimulus.dots.mu+stimulus.dots.rndAngle,1,stimulus.dots.n); %add noise to angle
% figure(1);hold on;
% hist(theta)
...
stimulus.dots.nFramesToReDraw = 2;
