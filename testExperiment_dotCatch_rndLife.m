function myscreen = testExperiment_dotCatch_rndLife(screenName)
%
% 2019.05.30. Modified from testExperiment_dotCatch.
...
% > In this version, at each frame, only dots that are alive are plotted. So the
% number of dots plotted would vary slightly from frame to frame and is an
% increasing function of dot lifespan (if dots live longer, on each frame there 
% would be less dots dying. testExperiment_dotCatch_rndLife_2 is the version that
% plots all dots by reseting dead dots.
%
%
...
% Draw dots still alive
stimulus.dots.xstep(~stimulus.dots.dead) = stimulus.dots.stepsize*cos(stimulus.dots.dir(~stimulus.dots.dead));
stimulus.dots.ystep(~stimulus.dots.dead) = stimulus.dots.stepsize*sin(stimulus.dots.dir(~stimulus.dots.dead));
    
% move those dots
stimulus.dots.x(~stimulus.dots.dead) = stimulus.dots.x(~stimulus.dots.dead)+stimulus.dots.xstep(~stimulus.dots.dead);
stimulus.dots.y(~stimulus.dots.dead) = stimulus.dots.y(~stimulus.dots.dead)+stimulus.dots.ystep(~stimulus.dots.dead);    
...
% draw the living dots
mglStencilSelect(1);
mglPoints2(stimulus.dots.x(~stimulus.dots.dead),stimulus.dots.y(~stimulus.dots.dead),stimulus.dots.dotsize,[1 1 1]);
