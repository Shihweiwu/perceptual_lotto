function myscreen = testExperiment_dotCatch_limitLife_2p11(screenName)
%
% Modified from testExperiment. dot-catching task.
% > Limit dots lifespan (e.g. 6 frames for 60Hz refresh rate = 100ms) so
% that after every 6 frames we will re-place dots position and redetermine all dots' motion path by resampling from
% the same von Mises. 
% > Add random motion.
%
...
stimulus.dots.nFramesToReDraw = 60;
...
%
% resample dots motion path for updateDots_2 by re-positioning all dots and resampling the direction of
% motion of a subset of dots (stimulus.dots.n_VM_dots) from von Mises.
%
